...
// 32.2  DIAGONAL WIPE into testimonial
diagWipe(32.2);
tl.set('#scene-testimonial', {{ opacity: 1 }}, 32.22);

tl.fromTo('#quote-mark', {{ opacity: 0, scale: 0.6 }}, {{ opacity: 1, scale: 1, duration: 0.5, ease: 'back.out(1.8)' }}, 32.35);
tl.fromTo('#quote-text', {{ opacity: 0, y: 22, filter: 'blur(8px)' }}, {{ opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.7, ease: 'power2.out' }}, 32.6);
tl.to('#quote-name', {{ opacity: 1, duration: 0.45 }}, 33.8);
tl.to('#quote-title', {{ opacity: 1, duration: 0.45 }}, 34.0);

tl.to('#scene-testimonial', {{ opacity: 0, duration: 0.4 }}, 36.2);
tl.to('#corner-logo', {{ opacity: 0, duration: 0.3 }}, 36.2);

// 36.6  ZOOM PUNCH into CTA
zoomPunch(36.6);
tl.set('#scene-cta', {{ opacity: 1 }}, 36.62);

tl.fromTo('#cta-logo', {{ scale: 0.7, opacity: 0 }}, {{ scale: 1, opacity: 1, duration: 0.55, ease: 'back.out(1.5)' }}, 36.75);
['ctaring1', 'ctaring2', 'ctaring3'].forEach((id, i) => {{
  gsap.set('#' + id, {{ opacity: 0 }});
  tl.to('#' + id, {{ opacity: 1, duration: 0.35 }}, 36.75);
  tl.to('#' + id, {{ scale: 1.03, duration: 1.1, repeat: 3, yoyo: true, ease: 'sine.inOut' }}, 36.9 + i * 0.08);
}});
tl.fromTo('#cta-headline', {{ opacity: 0, y: 22, filter: 'blur(8px)' }}, {{ opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.6, ease: 'power2.out' }}, 37.5);
flash(38.0, 0.28);
gsap.set('#cta-btn', {{ scale: 0.7 }});
tl.to('#cta-btn', {{ opacity: 1, scale: 1, duration: 0.45, ease: 'back.out(1.7)' }}, 38.0);
tl.to('#cta-shimmer', {{ left: '120%', duration: 1.1, repeat: 3, ease: 'power1.inOut' }}, 38.2);
tl.to('#cta-url', {{ opacity: 1, y: -4, duration: 0.45 }}, 38.6);

// 45.4 — fade out, loop
tl.to('#scene-cta', {{ opacity: 0, duration: 0.45 }}, 45.4);
tl.to('#blackout', {{ opacity: 1, duration: 0.9, ease: 'power2.in' }}, 45.6);
